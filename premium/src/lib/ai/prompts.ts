...
Clasifica las pruebas en "essential", "recommended" y "optional".
  `,
  DIAGNOSIS: `
### PRONTUARIO DE DIAGNÓSTICO FUNCIONAL P3 - Versión v3.6.2
...
1. **Más allá de lo Literal**: Infiere barreras o facilitadores de las actividades y ocupación (ej. sedentarismo, fatiga metabólica).
2. **Ponderación Contextual**: Considera Edad y Sexo para ajustar expectativas biológicas (reparación tisular) y riesgo.
3. **F3 Inferencia Clínica**: Conecta hallazgos estructurales con restricciones de participación (F1 -> F2).

### BLOQUE E — ESTRUCTURA Y FUNCIÓN:
**E1 — Checklist Estructural (genera un ítem por cada sistema afectado):**
- Nervio periférico, Músculo, Hueso, Cardiovascular, Respiratorio, Endocrino/Metabólico (Diabetes, Tiroides), Piel/Fascia.
**E2 — Checklist Funcional (genera 1 ítem por cada hallazgo):**
- Dolor (cada zona), Irritabilidad mecánica, Debilidad (cada grupo), Baja resistencia, Déficit de control motor, ROM limitado, Hipermovilidad, Baja tolerancia a la carga, Mecanosensibilidad neural, Balance/Propiocepción, Kinesiofobia, Stress/Sueño, Edema, Fatiga.

### BLOQUE F — ACTIVIDAD Y PARTICIPACIÓN:
**F1 — Limitaciones Directas (Tareas concretas):** Caminar, subir escaleras, sentarse, correr, yoga, etc. Use 'detalle' para explicar la biomecánica o síntoma relacionado.
**F2 — Restricciones de Participación (Roles/Contextos):** Trabajo, deporte, vida social, sexual, familiar. Use 'detalle' para explicar el impacto inferido.

### BLOQUE G — MATRIZ BIOPSICOSOCIAL INTEGRAL:
G1: Personales (+) Motivación, buena adherencia, resiliencia.
G2: Personales (-) Kinesiofobia, catastrofización, comorbilidades afectando ánimo o energía.
G3: Facilitadores Amb. Soporte social, acceso a gimnasio, seguro.
G4: Barreras Amb. Trabajo demandante/sedentario, escaleras, falta de tiempo.
G5: Moduladores Clínicos. Medicamentos (estatinas, levotiroxina), tabaquismo, cronicidad.
G6: Observaciones Integradas BPS. Síntesis narrativa conectando E, F y G.

### REGLAS TÉCNICAS CRÍTICAS:
- **FORMATO JSON PURO**: Sin markdown.
- **IDIOMA**: Español clínico.
- **ENUM DOLOR**: Usa exactamente uno de ['nociceptivo', 'neuropático', 'nociplástico', 'mixto', 'no_concluyente'] (todo minúsculas).
- **DATOS FALTANTES**: Nunca omitas propiedades obligatorias. Envía arrays vacíos [] si realmente no hay datos.
`,
