...
type DashboardData = {
  ...
  archivedParticipants: OlderAdultParticipant[];
...
export default function TallerAdultoMayorPage() {
  ...
  const setParticipantActive = async (participant: OlderAdultParticipant, active: boolean) => {
    if (!active && !confirm(`¿Archivar a ${participant.fullName}? Su historial se conservará.`)) return;
    await act(
      { action: 'setParticipantActive', participantId: participant.id, active },
      active ? 'Persona restaurada.' : 'Persona archivada.',
    );
  };
  const deleteTestParticipant = async (participant: OlderAdultParticipant) => {
    await act(
      { action: 'deleteTestParticipant', participantId: participant.id },
      'Persona de prueba y datos asociados eliminados.',
    );
  };
  const deleteDraft = async (evaluation: OlderAdultEvaluation) => {
    if (!confirm(`¿Descartar el borrador de ${evaluation.participantSnapshot.fullName}?`)) return;
    await act({ action: 'deleteDraftEvaluation', evaluationId: evaluation.id }, 'Borrador descartado.');
  };
  ...
  return (
    <div className="mx-auto max-w-7xl pb-20">
      ...
      {tab === 'PERSONAS' && <section className="mt-5 rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black text-slate-950">Personas del taller</h2><p className="text-sm text-slate-500">Una ficha simple, independiente del expediente clínico habitual.</p></div><button type="button" onClick={() => setEditing('NEW')} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white"><Plus className="h-4 w-4" /> Agregar persona</button></div>
        <div className="relative mt-5"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar por nombre o RUT" className="w-full rounded-2xl border border-slate-200 py-3 pl-11 pr-4" /></div>
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visibleParticipants.map(person => <article key={person.id} className={`rounded-2xl border p-4 ${person.testRecord ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-black text-slate-900">{person.fullName}</p><p className="mt-1 text-xs font-medium text-slate-500">{person.rut || 'Sin RUT'} · {person.age ? `${person.age} años` : 'Edad no registrada'}</p></div><button type="button" onClick={() => setEditing(person)} aria-label={`Editar ${person.fullName}`} className="rounded-xl bg-slate-100 p-2 text-slate-600"><Pencil className="h-4 w-4" /></button></div><div className="mt-3 flex flex-wrap gap-1.5 text-[10px] font-black uppercase"><span className="rounded-full bg-sky-50 px-2.5 py-1 text-sky-700">Lee: {person.readingAbility === 'SI' ? 'sí' : person.readingAbility === 'NO' ? 'no' : 'con dificultad'}</span><span className="rounded-full bg-violet-50 px-2.5 py-1 text-violet-700">Escribe: {person.writingAbility === 'SI' ? 'sí' : person.writingAbility === 'NO' ? 'no' : 'con dificultad'}</span></div><button type="button" onClick={() => startEvaluation(person.id)} disabled={busyId === `eval-${person.id}`} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-teal-50 px-3 py-2.5 text-xs font-black text-teal-800 disabled:opacity-40"><Activity className="h-4 w-4" /> Evaluar o reevaluar</button>{person.testRecord ? <button type="button" onClick={() => void deleteTestParticipant(person)} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-rose-600 px-3 py-2.5 text-xs font-black text-white"><Trash2 className="h-4 w-4" /> Eliminar prueba</button> : <button type="button" onClick={() => void setParticipantActive(person, false)} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-black text-slate-600"><Archive className="h-4 w-4" /> Archivar</button>}</article>)}</div>
        {!!data?.archivedParticipants.length && <div className="mt-7 border-t border-slate-200 pt-5"><h3 className="text-sm font-black text-slate-800">Personas archivadas</h3><div className="mt-3 flex flex-wrap gap-2">{data.archivedParticipants.map(person => <button key={person.id} type="button" onClick={() => void setParticipantActive(person, true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-black text-slate-600"><ArchiveRestore className="h-4 w-4" /> {person.fullName}</button>)}</div></div>}
      </section>}
