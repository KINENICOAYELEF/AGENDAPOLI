  ...
  return <div className="min-h-screen bg-slate-50 pb-40 sm:pb-32">
    ...
    <main className="mx-auto max-w-5xl space-y-4 p-3 sm:space-y-5 sm:p-6">
      ...
      {step === 2 && <Card eyebrow="Evaluación física focalizada" title="Seleccionar, medir e interpretar">
        <p className="text-xs text-slate-500">Marca solo los dominios que responden a la entrevista y a los objetivos.</p><fieldset className="mt-3 grid gap-2 sm:grid-cols-2"><legend className="sr-only">Dominios que se evaluarán</legend>{domains.map(domain => { const selected = data.exam.selectedDomains.includes(domain); return <label key={domain} className={`flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border p-3 text-sm font-semibold ${selected ? "border-indigo-500 bg-indigo-50 text-indigo-950" : "border-slate-200 bg-white text-slate-700"}`}><input type="checkbox" checked={selected} onChange={() => patch("exam", { selectedDomains: selected ? data.exam.selectedDomains.filter(item => item !== domain) : [...data.exam.selectedDomains, domain] })} className="h-5 w-5 shrink-0 accent-indigo-600" /><span>{domain}</span></label>; })}</fieldset>
        ...
        {data.exam.psfsScores.length > 0 && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-black text-amber-950">Actividades funcionales precargadas</p>{data.exam.psfsScores.map((item,index) => <div key={`${item.activity}-${index}`} className="mt-2 grid grid-cols-[1fr_auto] items-center gap-2 rounded-lg bg-white p-3"><span className="text-sm font-semibold">{item.activity}</span><label className="row-span-2 flex items-center gap-2 text-xs text-slate-500"><span>Ahora</span><input aria-label={`Puntuación actual ${item.activity}`} type="number" min="0" max="10" value={item.score} onChange={event => { const values=[...data.exam.psfsScores]; values[index]={...values[index],score:event.target.value}; patch("exam",{psfsScores:values}); }} className="h-12 w-16 rounded-lg border p-2 text-center text-base font-bold" /></label><span className="text-xs text-slate-500">Antes: {item.baselineScore ?? "—"}</span></div>)}</div>}
      </Card>}
      ...
      {step === 3 && <Card eyebrow="Integración clínica" title="Conectar entrevista, examen y plan">
        ...
        <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-black text-slate-900">Objetivos</p><button type="button" onClick={() => patch("reasoning", { objectives: [...data.reasoning.objectives, { id: generateId(), text: "", status: "NUEVO" }] })} className="min-h-11 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-bold text-white">Agregar objetivo</button></div><div className="mt-3 space-y-2">{data.reasoning.objectives.map((objective,index) => <div key={objective.id} className="grid gap-2 rounded-xl bg-white p-3 sm:grid-cols-[1fr_auto]"><input value={objective.text} onChange={event => { const values=[...data.reasoning.objectives]; values[index]={...objective,text:event.target.value}; patch("reasoning",{objectives:values}); }} placeholder="Objetivo observable y medible" className="min-h-12 rounded-lg border border-slate-200 p-3 text-base sm:text-sm" /><select value={objective.status} onChange={event => { const values=[...data.reasoning.objectives]; values[index]={...objective,status:event.target.value as Objective['status']}; patch("reasoning",{objectives:values}); }} className="min-h-12 rounded-lg border border-slate-200 p-3 text-base font-bold sm:text-sm"><option value="ACTIVO">Mantener</option><option value="LOGRADO">Logrado</option><option value="MODIFICADO">Modificar</option><option value="NUEVO">Nuevo</option></select></div>)}</div></div>
      </Card>}

    </main>
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-3 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur sm:px-6"><div className="mx-auto max-w-5xl"><div className="mb-2 flex items-center justify-between text-[11px] font-semibold"><span className={saving === "error" ? "text-rose-600" : "text-slate-500"}>{saving === "saving" ? "Guardando…" : saving === "saved" ? "Cambios guardados" : saving === "error" ? "No se pudo guardar" : "Autoguardado activo"}</span><button type="button" onClick={() => save(false)} className="min-h-8 px-2 font-bold text-indigo-700 underline decoration-indigo-200 underline-offset-4">Guardar ahora</button></div><div className="grid grid-cols-[minmax(96px,0.45fr)_1fr] gap-2"><button type="button" onClick={() => step === 1 ? onClose() : setStep((step - 1) as 1 | 2 | 3)} className="min-h-12 rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold text-slate-700">{step === 1 ? "Salir" : "Anterior"}</button>{step < 3 ? <button type="button" onClick={() => setStep((step + 1) as 1 | 2 | 3)} className="min-h-12 rounded-xl bg-indigo-600 px-4 text-sm font-black text-white shadow-sm">Continuar a {step === 1 ? "Examen" : "Decisión"}</button> : <button type="button" onClick={() => save(true)} className="min-h-12 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white shadow-sm">Finalizar y cerrar</button>}</div></div></div>
  </div>;
}
