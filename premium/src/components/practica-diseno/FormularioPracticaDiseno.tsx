...
// ─── COMPONENTE BLOQUE DE UN CASO CLÍNICO ──────────────────────────────────
function BloqueCasoClinico({
  numCaso,
  caso,
  onChange,
}: {
  ...
  const removeEvaluacion = (id: string) => {
    ...
    onChange({
      ...caso,
      evaluaciones: caso.evaluaciones.filter((ev) => ev.id !== id),
    });
  };
  ...
  const updateEvaluacion = (id: string, field: keyof EvaluacionDiseno, val: string) => {
    onChange({
      ...caso,
      evaluaciones: caso.evaluaciones.map((ev) => (ev.id === id ? { ...ev, [field]: val } : ev)),
    });
  };

  // Manejo de Objetivos Específicos
  ...
  const removeObjetivoEspecifico = (id: string) => {
    ...
    onChange({
      ...caso,
      objetivos: { ...caso.objetivos, especificos: reordered },
    });
  };
  ...
  const updateObjetivoEspecifico = (id: string, texto: string) => {
    onChange({
      ...
      objetivos: {
        ...caso.objetivos,
        especificos: caso.objetivos.especificos.map((obj) => (obj.id === id ? { ...obj, texto } : obj)),
      },
    });
  };

  // Manejo de Estrategias FITT-VP
  const addEstrategiaFitt = () => {
    if (caso.planIntervencion.estrategias.length >= 8) return;
    onChange({
      ...caso,
      planIntervencion: {
        estrategias: [...caso.planIntervencion.estrategias, estrategiaFittVacio()],
      },
    });
  };
  ...
  const removeEstrategiaFitt = (id: string) => {
    ...
    onChange({
      ...caso,
      planIntervencion: {
        estrategias: caso.planIntervencion.estrategias.filter((est) => est.id !== id),
      },
    });
  };
  ...
  const updateEstrategiaFitt = (id: string, field: keyof EstrategiaFittVP, val: string) => {
    onChange({
      ...caso,
      planIntervencion: {
        estrategias: caso.planIntervencion.estrategias.map((est) =>
  ...
  const toggleObjetivoRelacionado = (estId: string, objTag: string) => {
    ...
    onChange({
      ...caso,
      planIntervencion: {
        estrategias: caso.planIntervencion.estrategias.map((item) => {
          if (item.id !== estId) return item;
          return { ...item, objetivoRelacionado: nuevaRelacion };
        }),
      },
    });
  };
  ...
  const setCifItems = (key: keyof CasoDisenoIntervencion["cif"], items: CifItem[]) =>
    onChange({ ...caso, cif: { ...caso.cif, [key]: serializeCifItems(items) } });

  return (
    <div className="space-y-6">
      {/* 1. Datos de la Persona */}
      <SectionCard title={`1. Datos de la persona atendida (Caso #${numCaso})`}>
        <HelpText>
          Registren datos que permitan entender quién es la persona y por qué fue atendida. Resguarden la confidencialidad.
        </HelpText>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldInput
            ...
            onChange={(v) => onChange({ ...caso, datosUsuaria: { ...caso.datosUsuaria, nombre: v } })}
          ...
          <FieldInput
            ...
            onChange={(v) => onChange({ ...caso, datosUsuaria: { ...caso.datosUsuaria, edad: v } })}
          ...
          <FieldInput
            ...
            onChange={(v) => onChange({ ...caso, datosUsuaria: { ...caso.datosUsuaria, ocupacion: v } })}
          ...
          <FieldInput
            ...
            onChange={(v) => onChange({ ...caso, datosUsuaria: { ...caso.datosUsuaria, contextoAtencion: v } })}
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, datosUsuaria: { ...caso.datosUsuaria, motivoConsulta: v } })}
      ...
      {/* 2. Anamnesis */}
      <SectionCard title={`2. Anamnesis / Entrevista clínica realizada (Caso #${numCaso})`}>
        <GuideBox title="¿Qué se espera en esta sección?">
          <p>Escriban la <strong>entrevista clínica completa</strong> que realizaron como tratantes durante la jornada. Incluyan anamnesis próxima (problema actual o motivo de ingreso) y remota (antecedentes):</p>
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, anamnesis: v })}
          required
          rows={10}
          placeholder="Registren aquí la entrevista clínica completa realizada profesionalmente. Primero la anamnesis próxima (situación actual, funcionalidad, molestias o metas de salud). Luego la anamnesis remota (antecedentes médicos, comorbilidades, fármacos, caídas previas, hábitos y estilo de vida)..."
        />
        ...
        <div className="border-t border-slate-200 pt-5">
          ...
          <div className="mt-4">
            <FieldTA
              ...
              onChange={(v) => onChange({ ...caso, interpretacionAnamnesis: v })}
              required
              rows={4}
      ...
      {/* 3. Evaluaciones */}
      <SectionCard title={`3. Evaluaciones realizadas por el tratante (Caso #${numCaso})`}>
        <GuideBox title="¿Qué tipo de evaluaciones se esperan?">
          ...
          <p className="mt-1.5">Pueden ser pruebas de capacidad funcional, equilibrio (Romberg, Apoyo Unipodal, TUG), fuerza muscular o dinamometría, movilidad articular, marcha, tolerancia al esfuerzo, dolor o escalas funcionales.</p>
        </GuideBox>
      ...
      <SectionCard title={`4. Hallazgos principales del tratante (Caso #${numCaso})`}>
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, hallazgo1: v })}
          required
          rows={2}
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, hallazgo2: v })}
          required
          rows={2}
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, hallazgo3: v })}
          required
          rows={2}
      ...
      {/* 5. Tabla CIF */}
      <SectionCard title={`5. Tabla CIF – Clasificación del caso (Caso #${numCaso})`}>
        <GuideBox title="Guía para completar la CIF">
          ...
          <div className="mt-2 bg-white border border-blue-200 rounded-lg p-3 space-y-1">
            <p><strong>Estructuras</strong> = ¿Qué parte del cuerpo está comprometida o evaluada? (tejidos físicos, articulaciones, musculatura)</p>
            ...
            <p><strong>Actividades</strong> = ¿Qué tareas concretas están limitadas o requieren entrenamiento? (marcha, escaleras, transferencias)</p>
            ...
            <p><strong>F. Personales</strong> = ¿Qué factores propios facilitan (+) o dificultan (-)? (edad, motivación, autoeficacia, hábitos)</p>
            <p><strong>F. Ambientales</strong> = ¿Qué del entorno físico y social ayuda (+) o dificulta (-)? (red de apoyo, barreras, recintos)</p>
          </div>
        </GuideBox>
        ...
        <div className="space-y-4">
          <CifSection
            ...
            helpContent={<HelpText>Registren tejidos, articulaciones, musculatura o sistema neuromuscular involucrado.</HelpText>}
          ...
          <CifSection
            ...
            helpContent={<HelpText>Fuerza muscular, equilibrio estático/dinámico, movilidad articular, capacidad cardiorrespiratoria, dolor. <strong>Seleccionen severidad</strong>.</HelpText>}
          ...
          <CifSection
            ...
            helpContent={<HelpText>Marcha continua, subir/bajar escaleras, transferencias sedente a bípedo, agacharse. <strong>Seleccionen severidad</strong>.</HelpText>}
          ...
          <CifSection
            ...
            helpContent={<HelpText>Talleres comunitarios/deportivos, autonomía en la vida diaria, rol laboral o familiar. <strong>Seleccionen severidad</strong>.</HelpText>}
          ...
          <CifSection
            ...
            helpContent={<HelpText>Escriban si es facilitador (+) o barrera (-). Ej: (+) Alta motivación, adherencia | (-) Sedentarismo, temor a caerse.</HelpText>}
          ...
          <CifSection
            ...
            helpContent={<HelpText>Escriban si es facilitador (+) o barrera (-). Ej: (+) Apoyo familiar, acceso a centro deportivo | (-) Escaleras sin pasamanos.</HelpText>}
      ...
      <SectionCard title={`7. Objetivos de intervención (Caso #${numCaso})`}>
        {/* ACORDEÓN 1 */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                <div className="bg-red-50 p-2.5 rounded-lg border border-red-200 text-red-900">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 text-red-800">
                    ...
                    <li>&quot;Restablecer 20° de flexión de rodilla&quot; <em>(Es un específico analítico)</em></li>
                  </ul>
                </div>
          ...
          {showVerbosTable && (
            <div className="p-4 bg-white text-xs text-slate-700 space-y-3 border-t border-teal-200 overflow-x-auto">
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-[11px] text-amber-900 leading-relaxed">
                <strong>Regla de oro metodológica:</strong> Los verbos en los objetivos deben expresar el <strong>resultado o cambio fisiológico/funcional deseado</strong> (ej: <em>restablecer, modular, incrementar, reeducar, optimizar</em>). <strong>Nunca usen verbos operativos o de técnicas terapéuticas</strong> (ej: NO usar <em>elongar, masajear, traccionar ni aplicar frío</em>, ya que esos son procedimientos del plan de tratamiento, no metas).
              </div>
                ...
                <tbody className="divide-y divide-slate-200 text-[11px]">
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Dolor nociceptivo anterior de rodilla, dolor lumbopélvico mecánico, hiperalgesia, molestia en reposo / carga, umbral doloroso.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      a EVA ≤ 2/10, reducción de 3 puntos en EVA, abolición en reposo, presión &gt; 4 kg/cm².
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Rango de movimiento (ROM) articular activo/pasivo, extensibilidad miotendinosa de isquiotibiales/tríceps sural, juego articular.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      a +115° en flexión activa (goniometría), Sit and Reach a +5 cm, simetría &gt; 90%.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    <td className="border border-slate-200 p-2">
                      <strong className="text-teal-900 block font-semibold mb-1">Fuerza / Capacidad:</strong>
                      Desarrollar, incrementar, aumentar, potenciar, reclutar.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Fuerza isométrica/dinámica de cuádriceps y glúteo medio, potencia extensora de cadera, resistencia muscular local.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      a grado M4+ en escala MRC, incremento del 15% en dinamometría, sobrecarga submáxima.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Control lumbopélvico dinámico, alineación femorotibial frontal, estabilidad unipodal estática/dinámica, propiocepción.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      manteniendo apoyo unipodal &gt; 25s sin oscilaciones, evitando valgo dinámico.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Capacidad cardiorrespiratoria, tolerancia al esfuerzo submáximo, resistencia aeróbica, recuperación de FC.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      alcanzando &gt; 450 metros en TM6M, manteniendo esfuerzo percibido RPE ≤ 4 (Borg).
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Patrón cinemático de marcha, simetría del paso, fase de apoyo monopodal, cadencia y velocidad de marcha.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      velocidad de marcha ≥ 1.0 m/s, desplazamiento continuo de 500m sin asistencia ni claudicación.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Transición sedente a bípedo (sit-to-stand), transferencias piso-bípedo, agacharse a nivel de suelo, tiempo en TUG.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      alcanzando ≥ 12 reps en Chair Stand Test en 30s, reduciendo tiempo en TUG a &lt; 10 segundos.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                      en 3 semanas de programa.
                    </td>
                    <td className="border border-slate-200 p-2 italic text-slate-800 bg-slate-50">
                      &quot;Independizar la transición sedente a bípedo logrando ≥ 12 reps en Chair Stand Test en 3 semanas de programa.&quot;
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-slate-200 p-2">
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Estrategia de paso compensatorio, reacciones de enderezamiento postural ante perturbaciones, confianza en equilibrio.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      logrando paso protector eficaz ante perturbación externa, reduciendo temor a caer (FES-I).
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
                  ...
                  <tr>
                    ...
                    <td className="border border-slate-200 p-2 text-slate-700">
                      Autonomía en transporte público y compras vecinales, asistencia a talleres de envejecimiento activo, rol laboral.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-700">
                      asistiendo de forma autónoma y segura 2 veces por semana al taller, realizando trámites cotidianos.
                    </td>
                    <td className="border border-slate-200 p-2 text-slate-600">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-50/90 border border-amber-200 rounded-2xl p-4 space-y-2">
            ...
            <FieldTA
              ...
              onChange={(v) => onChange({ ...caso, objetivos: { ...caso.objetivos, problemaPrincipal: v } })}
          ...
          <div className="bg-emerald-50/90 border border-emerald-200 rounded-2xl p-4 space-y-2">
            ...
            <FieldTA
              ...
              onChange={(v) => onChange({ ...caso, objetivos: { ...caso.objetivos, objetivoGeneral: v } })}
      ...
      <SectionCard title={`8. Plan de intervención propuesto – Prescripción FITT-VP (Caso #${numCaso})`}>
        <GuideBox title="Prescripción y Dosificación FITT-VP">
          <p>Propongan las estrategias de intervención necesarias para dar cumplimiento a los objetivos planteados arriba.</p>
          <p className="mt-1">En cada estrategia, vinculen a qué <strong>Objetivo(s)</strong> tributa y completen los parámetros de dosificación:</p>
        </GuideBox>
              ...
              <div key={est.id} className="border border-slate-200 rounded-2xl p-5 bg-slate-50 space-y-4 relative shadow-sm">
                ...
                <div className="space-y-3">
                  <FieldInput
                    ...
                    placeholder="Ej: Ejercicio de fuerza en cuádriceps, entrenamiento de transferencias, educación postural..."
                  />

                  {/* SELECTOR INTERACTIVO DE OBJETIVOS VINCULADOS */}
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <FieldInput
                      ...
                      placeholder="Ej: Escala Borg RPE 4-6 (moderado a algo duro), Talk Test (sin ahogo), o peso/resistencia elástica"
                    />
                    <FieldInput
                    ...
                    <FieldInput
                      ...
                      placeholder="Ej: Avance: Aumentar reps o reducir base si RPE ≤ 3. Seguridad: Detener si EVA > 4 o mareo"
                    />
                  </div>
                </div>
              </div>
      ...
      <SectionCard title={`9. Pronóstico final y factores pronósticos (Caso #${numCaso})`}>
        <GuideBox title="¿Cómo determinar si el pronóstico es Favorable, Reservado o Desfavorable?">
          ...
          <div className="mt-2 space-y-2 text-slate-800">
            <div className="bg-white p-3 rounded-lg border border-green-200">
              ...
              <p className="mt-0.5">El cuadro es tratable/entrenable con kinesiología; factores protectores superan a barreras; buena motivación; no hay daño irreversible. Se espera recuperación o ganancia sustancial de autonomía.</p>
            </div>

            <div className="bg-white p-3 rounded-lg border border-amber-200">
              <span className="font-bold text-amber-700 uppercase tracking-wide">Pronóstico Reservado / Relativo:</span>
              <p className="mt-0.5">Incertidumbre clínica; comorbilidades crónicas o cuadro de larga data; balance equilibrado entre facilitadores y barreras. Requiere reevaluaciones periódicas.</p>
            </div>

            <div className="bg-white p-3 rounded-lg border border-red-200">
              <span className="font-bold text-red-700 uppercase tracking-wide">Pronóstico Desfavorable:</span>
              <p className="mt-0.5">Condición degenerativa severa o irreversible; predominio de barreras insalvables; alto riesgo de declive funcional severo.</p>
            </div>
          </div>
        </GuideBox>
        ...
        <FieldTA
          ...
          onChange={(v) => onChange({ ...caso, pronostico: { ...caso.pronostico, fundamentacion: v } })}
          placeholder="Fundamenten por qué clasificaron el pronóstico en esa categoría: analicen adaptabilidad tisular/neuromuscular, respuesta previa, comorbilidades y reserva funcional..."
        />
        ...
        <FieldTA
          ...
          onChange={(v) =>
            onChange({ ...caso, pronostico: { ...caso.pronostico, relacionDiagnosticoEIntervencion: v } })
          }
        ...
        <div className="pt-2">
          ...
          <div className="space-y-3">
            <FieldInput
              ...
              onChange={(v) => onChange({ ...caso, pronostico: { ...caso.pronostico, factorPronostico1: v } })}
            ...
            <FieldInput
              ...
              onChange={(v) => onChange({ ...caso, pronostico: { ...caso.pronostico, factorPronostico2: v } })}
            ...
            <FieldInput
              ...
              onChange={(v) => onChange({ ...caso, pronostico: { ...caso.pronostico, factorPronostico3: v } })}
...
function validarCaso(c: CasoDisenoIntervencion, num: number): string[] {
  const errs: string[] = [];
  const pref = `Caso #${num}`;

  if (!c.datosUsuaria.nombre.trim()) errs.push(`${pref}: Nombre o iniciales de la persona atendida.`);
  if (!c.datosUsuaria.edad.trim()) errs.push(`${pref}: Edad de la persona.`);
  if (!c.datosUsuaria.motivoConsulta.trim()) errs.push(`${pref}: Motivo de consulta.`);
  if (!c.anamnesis.trim()) errs.push(`${pref}: Anamnesis / Entrevista clínica.`);
  if (!c.interpretacionAnamnesis.trim()) errs.push(`${pref}: Interpretación de la anamnesis.`);
  if (!c.enunciadoDiagnostico.trim()) errs.push(`${pref}: Diagnóstico kinesiológico incipiente.`);
  if (!c.objetivos.problemaPrincipal.trim()) errs.push(`${pref}: Problema kinesiológico principal.`);
  if (!c.objetivos.objetivoGeneral.trim()) errs.push(`${pref}: Objetivo general de intervención.`);

  const algunObjVacio = c.objetivos.especificos.some((obj) => !obj.texto.trim());
  if (c.objetivos.especificos.length === 0 || algunObjVacio) {
    errs.push(`${pref}: Texto de todos los objetivos específicos priorizados.`);
  }

  const algunaEstIncompleta = c.planIntervencion.estrategias.some(
    (e) => !e.nombreEstrategia.trim() || !e.frecuencia.trim() || !e.intensidad.trim()
  );
  if (c.planIntervencion.estrategias.length === 0 || algunaEstIncompleta) {
    errs.push(`${pref}: Modalidad, frecuencia e intensidad en todas las estrategias FITT-VP.`);
  }

  if (!c.pronostico.calificacion) {
    errs.push(`${pref}: Clasificación del pronóstico (Favorable, Reservado o Desfavorable).`);
  }
  if (!c.pronostico.factorPronostico1.trim() || !c.pronostico.factorPronostico2.trim() || !c.pronostico.factorPronostico3.trim()) {
    errs.push(`${pref}: Declaración de los 3 factores pronósticos obligatorios.`);
  }

  return errs;
}

// ─── FORMULARIO PRINCIPAL CON 2 CASOS Y TABS ────────────────────────────────
export default function FormularioPracticaDiseno() {
  const STORAGE_KEY = "practica_diseno_draft_2casos_v1";
  ...
  const [caso1, setCaso1] = useState<CasoDisenoIntervencion>(casoDisenoVacio());
  const [caso2, setCaso2] = useState<CasoDisenoIntervencion>(casoDisenoVacio());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errsDupla: string[] = [];
    if (!dupla.estudiante1.trim()) errsDupla.push("Nombre del Estudiante 1");
    ...
    try {
      ...
      console.error(err);
      setErrores(["Ocurrió un error al enviar la práctica. Por favor intenta de nuevo."]);
    } finally {
      setEnviando(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {errores.length > 0 && (
        <div className="mb-6 p-5 bg-red-50 border border-red-300 rounded-2xl text-red-800 text-xs space-y-2">
          <p className="font-bold text-sm text-red-900">Por favor completa los siguientes campos obligatorios antes de enviar:</p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {tabActivo === 1 ? (
          <BloqueCasoClinico numCaso={1} caso={caso1} onChange={setCaso1} />
        ) : (
          <BloqueCasoClinico numCaso={2} caso={caso2} onChange={setCaso2} />
        )}

        {/* BOTONES DE NAVEGACIÓN Y ENVÍO */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pb-16">
          <div className="flex gap-3 w-full sm:w-auto">
            {tabActivo === 1 ? (
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-xl transition"
              >
                Pasar a Caso Clínico #2 →
              </button>
            ) : (
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={enviando}
            className="w-full sm:w-auto px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-base rounded-2xl shadow-lg hover:shadow-xl transition disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {enviando ? (
              <>
